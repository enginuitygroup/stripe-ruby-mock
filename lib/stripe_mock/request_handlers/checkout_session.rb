    ...
    module Checkout
      module Session
        ...
        def new_session(route, method_url, params, headers)
          ...
          require_param(:return_url) if params[:ui_mode] == 'embedded' && (params[:return_url].nil? || params[:return_url].empty?)
          require_param(:success_url) if params[:ui_mode] != 'embedded' && (params[:success_url].nil? || params[:success_url].empty?)
